...
static unsigned int e1functiondebounce=0;

static void e1FunctionAlert() {
    if(millis()>=e1functiondebounce) {
        int level=digitalRead(E1_FUNCTION);
        if(level==0) {
          if(running) g_idle_add(e_function_pressed,(gpointer)1);
        }
        e1functiondebounce=millis()+SETTLE_TIME;
    }
}

static unsigned int e2functiondebounce=0;

static void e2FunctionAlert() {
    if(millis()>=e2functiondebounce) {
        int level=digitalRead(E2_FUNCTION);
        if(level==0) {
          if(running) g_idle_add(e_function_pressed,(gpointer)2);
        }
        e2functiondebounce=millis()+SETTLE_TIME;
    }
}

static unsigned int e3functiondebounce=0;
  
static void e3FunctionAlert() {
    if(millis()>=e3functiondebounce) {
        int level=digitalRead(E3_FUNCTION);
        if(level==0) {
          if(running) g_idle_add(e_function_pressed,(gpointer)3);
        }
        e3functiondebounce=millis()+SETTLE_TIME;
    }
}

static unsigned int functiondebounce=0;

static void functionAlert() {
    if(millis()>=functiondebounce) {
        int level=digitalRead(FUNCTION_BUTTON);
        if(level==0) {
          if(running) g_idle_add(function_pressed,NULL);
        }
        functiondebounce=millis()+SETTLE_TIME;
    }
}


static unsigned int s1debounce=0;

static void s1Alert() {
    if(millis()>=s1debounce) {
        ...
        s1debounce=millis()+SETTLE_TIME;
    }
}

static unsigned int s2debounce=0;

static void s2Alert() {
    if(millis()>=s2debounce) {
        ...
        s2debounce=millis()+SETTLE_TIME;
    }
}

static unsigned int s3debounce=0;

static void s3Alert() {
    if(millis()>=s3debounce) {
        ...
        s3debounce=millis()+SETTLE_TIME;
    }
}

static unsigned int s4debounce=0;

static void s4Alert() {
    if(millis()>=s4debounce) {
        ...
        s4debounce=millis()+SETTLE_TIME;
    }
}

static unsigned int s5debounce=0;

static void s5Alert() {
    if(millis()>=s5debounce) {
        ...
        s5debounce=millis()+SETTLE_TIME;
    }
}

static unsigned int s6debounce=0;

static void s6Alert() {
    if(millis()>=s6debounce) {
...
/*
  if(piHiPri(99)!=0) {
    fprintf(stderr,"gpio_wiringpi: piHiPri returned %d\n", errno);
  }
*/
