    ...
    switch(radio->protocol) {
      ...
      case NEW_PROTOCOL:
        ...
        if(d->device==DEVICE_OZY) {
          sprintf(text,"%s (%s) on USB /dev/ozy\n", radio->name, radio->protocol==ORIGINAL_PROTOCOL?"old":"new");
        } else {
#endif
          sprintf(text,"Starting %s (%s v%d.%d)",
                        ...
                        radio->software_version%10);
/*
          sprintf(text,"Starting %s (%s %d.%d) %s (%02X:%02X:%02X:%02X:%02X:%02X)",
                        radio->name,
...
//  g_object_ref((gpointer)transmitter->panel);
  ...
  if(device!=DEVICE_METIS) {
    receiver[PS_RX_FEEDBACK]=create_pure_signal_receiver(PS_RX_FEEDBACK, buffer_size,protocol==ORIGINAL_PROTOCOL?active_receiver->sample_rate:192000);
    receiver[PS_TX_FEEDBACK]=create_pure_signal_receiver(PS_TX_FEEDBACK, buffer_size,protocol==ORIGINAL_PROTOCOL?active_receiver->sample_rate:192000);
  }
...
fprintf(stderr,"create sliders\n");
  ...
  gtk_widget_hide(transmitter->panel);
//#ifdef FREEDV
...
static void rxtx(int state) {
  ...
  if(state) {
    ...
    for(i=0;i<receivers;i++) {
      ...
      gtk_widget_hide(receiver[i]->panel);
    }
    gtk_widget_show(transmitter->panel);
//#ifdef FREEDV
//    if(active_receiver->freedv) {
...
//#endif
    gtk_widget_hide(transmitter->panel);
    for(i=0;i<receivers;i++) {
      gtk_widget_show(receiver[i]->panel);
...
void setSquelch(RECEIVER *rx) {
  double sq=((rx->squelch/100.0)*160.0)-160.0;
  SetRXAAMSQThreshold(rx->id, sq);
  SetRXAAMSQRun(rx->id, rx->squelch_enable);

fprintf(stderr,"setSquelch: squelch=%f AM Threshold=%f enable=%d\n",rx->squelch,sq,rx->squelch_enable);

  sq=pow(10.0, -2.0*rx->squelch/100.0);
  SetRXAFMSQThreshold(rx->id, sq);
  SetRXAFMSQRun(rx->id, rx->squelch_enable);
fprintf(stderr,"setSquelch: squelch=%f FM Threshold=%f enable=%d\n",rx->squelch,sq,rx->squelch_enable);
}
