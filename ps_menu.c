    ...
    if(isTransmitting()) {
      ...
      for(i=0;i<INFO_SIZE;i++) {
        ...
        switch(i) {
          ...
          case 15:
            switch(info[i]) {
              case 0: sprintf(label,"%s","RESET");
                break;
              case 1: sprintf(label,"%s","WAIT");
                break;
              case 2: sprintf(label,"%s","MOXDELAY");
                break;
              case 3: sprintf(label,"%s","SETUP");
                break;
              case 4: sprintf(label,"%s","COLLECT");
                break;
              case 5: sprintf(label,"%s","MOXCHECK");
                break;
              case 6: sprintf(label,"%s","CALC");
                break;
              case 7: sprintf(label,"%s","DELAY");
                break;
              case 8: sprintf(label,"%s","STAYON");
                break;
              case 9: sprintf(label,"%s","TURNON");
                break;
              default:
                sprintf(label,"UNKNOWN=%d",info[i]);
                break;
            }
            break;
          default:
...
//fprintf(stderr,"%s\n",label);
      ...
      gtk_entry_set_text(GTK_ENTRY(get_pk),pk_text);
...
//fprintf(stderr,"ps_menu: newcal=%d old5_2=%d info[5]=%d\n",newcal,old5_2,info[5]);
        old5_2=info[5];
        switch(state) {
          case 0:
...
//fprintf(stderr,"ps_menu: info_thread: exit\n");
}
